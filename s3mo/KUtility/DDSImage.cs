...
public class DDSImage
{
    ...
    public int dwMagic;
    private readonly DDS_HEADER header = new();
    private readonly DDS_HEADER_DXT10 header10; //If the DDS_PIXELFORMAT dwFlags is set to DDPF_FOURCC and dwFourCC is set to "DX10"
    ...
    public DDSImage(byte[] rawdata)
    {
        ...
        if (
            ...
            && (
                header.ddspf.dwFourCC == FOURCC_DX10 /*DX10*/
            )
        )
        {
    ...
    private Bitmap readBlockImage(byte[] data, int w, int h)
    {
        ...
        throw new Exception(
            string.Format(
                "0x{0} texture compression not implemented.",
    ...
    {
        Bitmap res = new((w < 4) ? 4 : w, (h < 4) ? 4 : h);
    ...
    private void DecompressBlockDXT1(int x, int y, byte[] blockStorage, Bitmap image)
    {
        ...
        uint code = (uint)(
            blockStorage[4] | blockStorage[5] << 8 | blockStorage[6] << 16 | blockStorage[7] << 24
        );
    ...
    void DecompressBlockDXT5(int x, int y, byte[] blockStorage, Bitmap image)
    {
        ...
        int bitOffset = 2;
        uint alphaCode1 = (uint)(
            blockStorage[bitOffset + 2]
            | (blockStorage[bitOffset + 3] << 8)
            | (blockStorage[bitOffset + 4] << 16)
            | (blockStorage[bitOffset + 5] << 24)
        );
        ushort alphaCode2 = (ushort)(
            blockStorage[bitOffset + 0] | (blockStorage[bitOffset + 1] << 8)
        );
