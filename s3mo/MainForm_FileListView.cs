...
{
    readonly List<string> fileListView_conflictLosingPackageModel = [];
    readonly List<string> fileListView_conflictWinningPackageModel = [];
    ...
    )
    {
        ...
        if (modListView.SelectedItems.Count != 1)
        {
            throw new InvalidOperationException(
                "Selected items is not 1. Right panel should not show anything so clicking is impossible"
            );
        }
    ...
    private void fileListView_DrawColumnHeader(
        object sender,
    ...
    private void fileListView_DrawSubItem(object sender, DrawListViewSubItemEventArgs e)
    {
        ...
        else if (e.SubItem == e.Item.SubItems[1])
        {
            ...
            {
                throw new Exception(
                    $"'{packageModel.Name}' is conflicting with package(s) but couldn't determine its conflict status"
                );
            }
        }
    }
    ...
    private void fileListView_MouseDoubleClick(object sender, MouseEventArgs e)
    {
        ...
        if (
            listViewItem == null
            || !Path.GetExtension(listViewItem.Text)
        ...
        if (
            !_profileModel
                .GetModModel(modListView.SelectedItems[0].Text)
                .TryGetPackageModel(listViewItem.Text, out PackageModel packageModel)
        )
        {
            throw new Exception(
                $"Couldn't get packageModel from {modListView.SelectedItems[0].Text}"
            );
        }

        Enabled = false;
        ModForm modForm = new(packageModel);
