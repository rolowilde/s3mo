...
public partial class MainForm : Form
{
    ...
    {
        InitializeComponent();
        Text = "Sims 3 Mod Organizer v" + Program.Version;
    ...
    private void MainForm_FormClosing(object sender, FormClosingEventArgs e)
    {
        ...
        Settings.SetSettings(
            "WindowSplitContainer1SplitterDistance",
            splitContainer1.SplitterDistance
        );
        Settings.SetSettings(
            "WindowSplitContainer2SplitterDistance",
    ...
    {
        if (
            Settings.TryGetSettings("WindowX", out string obj)
            && int.TryParse(obj, out int i)
            && Settings.TryGetSettings("WindowY", out string obj2)
        ...
        if (
            Settings.TryGetSettings("WindowWidth", out obj)
            && int.TryParse(obj, out i)
            && Settings.TryGetSettings("WindowHeight", out obj2)
        ...
        if (
            Settings.TryGetSettings("WindowSplitContainer1SplitterDistance", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("WindowSplitContainer2SplitterDistance", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("WindowModListViewModHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
            modHeader.Width = i;
        ...
        if (
            Settings.TryGetSettings("WindowModListViewFlagHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
            flagHeader.Width = i;
        ...
        if (
            Settings.TryGetSettings("WindowModListViewPriorityHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("WindowFileListViewFileHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
            fileHeader.Width = i;
        ...
        if (
            Settings.TryGetSettings("WindowFileListViewFlagHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        Logger.InfoLoggedEvent += s =>
        {
            logListBox.Items.Add("[" + DateTime.Now.ToString("HH:mm:ss") + "] " + s);
        ...
        Logger.WarningLoggedEvent += s =>
        {
            logListBox.Items.Add("[" + DateTime.Now.ToString("HH:mm:ss") + "] " + s);
            logListBox.TopIndex = logListBox.Items.Count - 1;
        };
        Logger.ErrorLoggedEvent += s =>
        {
            logListBox.Items.Add("[" + DateTime.Now.ToString("HH:mm:ss") + "] " + s);
            logListBox.TopIndex = logListBox.Items.Count - 1;
        };
    ...
    private void RefreshListViews(bool setupModel = true)
    {
        ...
        string[] profiles = Directory.GetDirectories(
    ...
    private void BuildMods()
    {
        ...
        {
            MessageBox.Show(
                "No Sims 3 Documents folder path is entered in settings, aborting merge process.",
                "Invalid Folder Path",
        ...
        StreamWriter w = new(resourceCfgPath);

        string buildPriority = Settings.TryGetSettings("BuildPriority", out string obj)
            ? obj
            : "495";

        string[] modSetupLines =
        [
        ...
        try
        {
            stream = new FileStream(
                Path.Combine(modBuildFolderPath, "Merge.bin"),
                FileMode.Create,
                FileAccess.Write,
                FileShare.None
            );
        ...
        catch (Exception ex)
        {
            MessageBox.Show(
                ex.Message,
                "Could not write packages meta. Aborting.",
                MessageBoxButtons.OK,
                MessageBoxIcon.Error
            );
            return;
        ...
        foreach (ModModel mod in mods)
        {
            ...
            foreach (PackageModel packageModel in packageModels)
            {
                ...
                while (
                    checkDetailedConflictBetweenModModelBackgroundWorker.IsBusy
                    || checkDetailedConflictBetweenModModelBackgroundWorker.IsBusy
                )
                {
                ...
                if (packageModel.Hash == null)
                {
                    ...
                    Enabled = true;
                    MessageBox.Show(
                        "Wait for MD5 hashbytes to compute or refresh if it's not computing.",
                        "No hashbyte to write",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Error
                    );
        ...
        BackgroundWorker mergeBackgroundWorker = new();
        mergeBackgroundWorker.WorkerReportsProgress = true;
        mergeBackgroundWorker.DoWork += (s, e) =>
            Package.Merge(
                modBuildFolderPath,
                packagesToMerge,
                buildLimit,
                i => mergeBackgroundWorker.ReportProgress(i)
            );
        mergeBackgroundWorker.ProgressChanged += (s, e) =>
        {
            string text =
                $"Merging {Path.GetFileName(packagesToMerge[e.ProgressPercentage].FilePath)}";
            Logger.Log(text);
        ...
        {
            Logger.Log(
                $"Build process finished in {stopwatch.ElapsedMilliseconds} ms for {count} packages."
            );
            stopwatch.Stop();
    ...
    private void profileComboBox_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (profileName == null || _currentProfile.Equals(profileName))
        {
            return;
        }
    ...
    private void optionsButton_Click(object sender, EventArgs e)
    {
        ...
        settingsForm.FormClosed += (s, e) => Enabled = true;
        Enabled = false;
        settingsForm.Show();
    }
    ...
    {
        ProfileForm form = new(_currentProfile);
        Enabled = false;
        form.FormClosed += (s, e) =>
        {
            RefreshListViews();
    ...
    private void clearCacheButton_Click(object sender, EventArgs e)
    {
        ...
        List<string> deleteFolderList = [];

        if (
            Settings.TryGetSettings("ClearCASPartCache", out string obj)
        ...
        if (
            Settings.TryGetSettings("ClearCompositorCache", out obj)
            && bool.TryParse(obj, out b)
            && b
        )
        {
        ...
        if (
            Settings.TryGetSettings("ClearSimCompositorCache", out obj)
            && bool.TryParse(obj, out b)
            && b
        )
        {
        ...
        if (
            Settings.TryGetSettings("ClearDCCacheMissingdeps", out obj)
            && bool.TryParse(obj, out b)
            && b
        )
        {
        ...
        if (
            Settings.TryGetSettings("ClearFeaturedItems", out obj)
            && bool.TryParse(obj, out b)
            && b
        )
        {
        ...
        if (
            Settings.TryGetSettings("ClearSavedSimsDownloadedSims", out obj)
            && bool.TryParse(obj, out b)
            && b
        )
        {
        ...
        if (
            Settings.TryGetSettings("ClearNRaasScriptErrors", out obj)
            && bool.TryParse(obj, out b)
            && b
        )
        {
