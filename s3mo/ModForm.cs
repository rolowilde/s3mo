...
{
    readonly ModModel _modModel = null!;

    readonly PackageModel _packageModel = null!;
    readonly Package _package = null!;

    readonly List<ResourceConflictModel> _losingResourceModels = [];
    readonly List<ResourceConflictModel> _winningResourceModels = [];
    ...
    private void ModForm_FormClosing(object sender, FormClosingEventArgs e)
    {
        ...
        Settings.SetSettings(
            "ModFormPackageSplitContainerSplitterDistance",
            packageSplitContainer.SplitterDistance
        );
        ...
        Settings.SetSettings(
            "ModFormResourceSplitContainerSplitterDistance",
            resourceSplitContainer.SplitterDistance
        );
    ...
    {
        if (
            Settings.TryGetSettings("ModFormX", out string obj)
            && Settings.TryGetSettings("ModFormY", out string obj2)
            && int.TryParse(obj, out int i)
            && int.TryParse(obj2, out int j)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormWidth", out obj)
            && Settings.TryGetSettings("ModFormHeight", out obj2)
            && int.TryParse(obj, out i)
            && int.TryParse(obj2, out j)
        )
        {
            Size = new Size(i, j);
        }
        ...
        if (
            Settings.TryGetSettings("ModFormResourceNameHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormResourceTagHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormResourceTypeHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormResourceGroupHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormResourceInstanceHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormLosingTagHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormLosingResourceHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormLosingPackageHeader1Width", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormLosingBlankHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormLosingModHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormLosingPackageHeader2Width", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormWinningTagHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormWinningResourceHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormWinningPackageHeader1Width", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormWinningBlankHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormWinningModHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormWinningPackageHeader2Width", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        {
            int width =
                losingListView.Size.Width
                - losingTagHeader.Width
                - losingResourceHeader.Width
                - losingBlankHeader.Width
                - losingModHeader.Width
                - 21;
            ...
            width =
                winningListView.Size.Width
                - winningTagHeader.Width
                - winningResourceHeader.Width
                - winningBlankHeader.Width
                - winningModHeader.Width
                - 21;
            width = (int)((float)width * 0.5f);
    ...
    private void CalculateConflictData()
    {
        ...
        foreach (PackageModel packageModel in packageModels)
        {
            ...
            {
                List<PackageModel> conflictingPackageModels =
                    packageModel.GetConflictingPackageModelsByResource(resource);
                ...
                foreach (PackageModel conflictingPackageModel in conflictingPackageModels)
                {
                    ...
                    ResourceConflictModel resourceModel = new(
                        resource,
                        packageModel.Name,
                        conflictingPackageModel.ModModel.Name,
                        conflictingPackageModel.Name
                    );
                    ...
                    if (packageModel.ModModel.Priority > conflictingPackageModel.ModModel.Priority)
                    {
                        ...
                        packageModel.ModModel.Priority < conflictingPackageModel.ModModel.Priority
                    )
                    {
                    ...
                    {
                        throw new Exception(
                            $"Conflict status unknown between {packageModel.Name} & {conflictingPackageModel.Name}"
                        );
                    }
                }
            }
    ...
    {
        List<PackageModel> conflictingPackageModels =
            _packageModel.ConflictingPackageModelsInModModel;
        ...
        foreach (PackageModel conflictingPackageModel in conflictingPackageModels)
        {
            ...
            if (
                !_packageModel.Package.IsConflictingWith(
                    conflictingPackageModel.Package,
                    out var resourceEntries,
                    out _,
                    false
                )
            )
            ...
            foreach (ResourceEntry resourceEntry in resourceEntries)
            {
                ...
                {
                    throw new Exception(
                        $"Conflict status unknown between {_packageModel.Name} & {conflictingPackageModel.Name}"
                    );
                }
            }
        }
        ...
        losingGroupBox.Text =
            "The list below shows the resources in other packages in the same mod that will be replaced by this package";
        winningGroupBox.Text =
            "The list below shows the resources in other packages in the same mod that will replace this package";
        PopulateListView();
    }
    ...
    private void PopulateListView()
    {
        ...
        {
            ListViewItem item = new(resourceModel.ResourceKey);
            if (
                Helper.TagList.TryGetValue(
                    resourceModel.ResourceKey.Substring(0, 10),
                    out string? type
                )
            )
            {
            ...
            item.SubItems.Add(
                resourceModel.PackageName.Substring(
                    0,
                    resourceModel.PackageName.LastIndexOf(".package")
                )
            );
            item.SubItems.Add(string.Empty);
            item.SubItems.Add(resourceModel.OtherModName);
            item.SubItems.Add(
                resourceModel.OtherPackageName.Substring(
                    0,
                    resourceModel.OtherPackageName.LastIndexOf(".package")
                )
            );
    ...
    private void losingListView_DrawSubItem(object sender, DrawListViewSubItemEventArgs e)
    {
        if (e.ItemIndex > 2)
        {
            ...
            {
                e.SubItem!.BackColor = prevItem.BackColor.Equals(SystemColors.Window)
        ...
        {
            e.SubItem!.BackColor =
                (e.ItemIndex % 2 != 0) ? SystemColors.Control : SystemColors.Window;
        }

        e.DrawDefault = true;
    }

    private void winningListView_DrawColumnHeader(
        object sender,
    ...
    private readonly List<ResourceModel> _resourceModels = [];
    private readonly Dictionary<string, string> _nameMapLookup = [];
    private readonly Dictionary<string, Bitmap> _ddsBitmaps = [];
    ...
    private void PopulateResourceContent()
    {
        ...
        if (
            Settings.TryGetSettings("ModFormResourceSplitContainerSplitterDistance", out string obj)
            && int.TryParse(obj, out int i)
        )
        {
    ...
    private void resourceDataBackgroundWorker_ProgressChanged(
        object sender,
    ...
    )
    {
        ...
        if (
            resourceListView.SelectedItems.Count == 1
            && e.ProgressPercentage > 0
            && resourceListView
                .SelectedItems[0]
                .Text.Equals(_resourceModels[e.ProgressPercentage - 1].ResourceKey)
        )
        {
    ...
    private void resourceDataBackgroundWorker_RunWorkerCompleted(
        object sender,
    ...
    private void resourceListView_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (Helper.ImageTypes.Contains(type))
        {
            ...
            MemoryStream stream = new(resourceModel.Data);
    ...
    private void PopulateModContent()
    {
        ...
        if (
            Settings.TryGetSettings("ModFormPackageSplitContainerSplitterDistance", out string obj)
            && int.TryParse(obj, out int i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormPackageSplitContainerSplitterDistance", out obj)
            && int.TryParse(obj, out i)
        )
        {
            packageSplitContainer.SplitterDistance = i;
        ...
        if (
            Settings.TryGetSettings("ModFormPackageHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        if (
            Settings.TryGetSettings("ModFormPackageFlagHeaderWidth", out obj)
            && int.TryParse(obj, out i)
        )
        {
        ...
        foreach (PackageModel packageModel in packageModels)
        {
            ListViewItem item = new(packageModel.Name);
            item.SubItems.Add(string.Empty);
            packageListView.Items.Add(item);
        }
    }

    readonly List<string> packageListView_conflictLoser = [];
    readonly List<string> packageListView_conflictWinner = [];
    ...
    private void packageListView_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        List<PackageModel> conflictingPackageModels =
            packageModel.ConflictingPackageModelsInModModel;
    ...
    private void packageListView_DrawColumnHeader(
        object sender,
        DrawListViewColumnHeaderEventArgs e
    )
    {
    ...
    private void packageListView_DrawSubItem(object sender, DrawListViewSubItemEventArgs e)
    {
        ...
        {
            e.SubItem!.BackColor =
                (e.ItemIndex % 2 == 0) ? SystemColors.Window : SystemColors.Control;
        }
        ...
        if (e.SubItem == e.Item!.SubItems[0])
        {
            e.Graphics.DrawString(
                e.SubItem.Text,
                Font,
                e.Item.Selected ? Brushes.White : Brushes.Black,
                e.Bounds.X + 5,
                e.Bounds.Y + 1
            );
        }
        else if (e.SubItem == e.Item.SubItems[1])
        {
            ...
            conflictingPackageModels.Sort(
                (p1, p2) => StringLogicalComparer.Compare(p1.Name, p2.Name)
            );

            Rectangle imageRect = new(
                e.Bounds.X + (int)(e.Bounds.Width / 2f) - 8,
                e.Bounds.Y + (int)(e.Bounds.Height / 2f) - 8,
                16,
                16
            );

            int i = StringLogicalComparer.Compare(
                packageModel.Name,
                conflictingPackageModels[0].Name
            );
            int j = StringLogicalComparer.Compare(
                packageModel.Name,
                conflictingPackageModels[conflictingPackageModels.Count - 1].Name
            );
    ...
    private void packageListView_MouseDoubleClick(object sender, MouseEventArgs e)
    {
        ...
        if (hit.Item != null)
        {
            PackageModel? packageModel = _modModel.PackageModels.Find(p =>
                hit.Item.Text.Equals(p.Name)
            );
