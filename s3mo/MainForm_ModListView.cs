...
partial class MainForm
{
    ...
    private bool modListView_isEditingText; // Use to hide the drawn string on subitem.
    ...
    private void modListView_AfterLabelEdit(object sender, LabelEditEventArgs e)
    {
        ...
        Logger.LogDebug(
            $"Renaming mod folder from '{modListView_preLabelEditBuffer}' to '{newFolderName}'"
        );
        _profileModel.RenameMod(modListView_preLabelEditBuffer, newFolderName);
    }

    private readonly List<string> modListView_losingConflictingModModels = [];
    private readonly List<string> modListView_winningConflictingModModels = [];

    private void modListView_ItemSelectionChanged(
        object sender,
    ...
    )
    {
        ...
        {
            ListViewItem listViewItem = new(packageModel.Name);
            listViewItem.SubItems.Add(string.Empty);
            fileListView.Items.Add(listViewItem);
        }

        string[] otherFiles = Directory
            .GetFiles(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, $"Mods\\{e.Item?.Text}"))
            .Where(f => !f.EndsWith(".package"))
            .Select(p => Path.GetFileName(p))
            .ToArray();

        foreach (string otherFile in otherFiles)
        {
            ListViewItem listViewItem = new(otherFile);
            listViewItem.SubItems.Add(string.Empty);
            fileListView.Items.Add(listViewItem);
        }
    ...
    private void modListView_DrawSubItem(object sender, DrawListViewSubItemEventArgs e)
    {
        ...
        if (e.SubItem == e.Item.SubItems[0]) // Name
        {
            CheckBoxRenderer.DrawCheckBox(
                e.Graphics,
                new Point(e.Bounds.X + 7, e.Bounds.Y + 3),
                e.Item.Checked
        ...
        else if (e.SubItem == e.Item.SubItems[2]) // Priority
        {
            e.Graphics.DrawString(
                ...
                Font,
                e.Item.Selected ? Brushes.White : Brushes.Black,
                e.Bounds.X - (4 * e.SubItem.Text.Length) + e.Bounds.Width / 2,
                e.Bounds.Y + 1
            );
        }
        else if (e.SubItem == e.Item.SubItems[1]) // Icons
        {
            ...
            List<ModModel> conflictingModModels = modModel.ConflictingModModels.FindAll(m =>
                m.Enabled
            );
            ...
            if (
                modModel.PackageModels.Count(p => p.ConflictingPackageModelsInModModel.Count > 0)
                > 0
            )
            {
                e.Graphics.DrawImage(
                    (Image)Properties.Resources.package_minus,
                    e.Bounds.X + 2,
                    e.Bounds.Y,
                    16,
                    16
            ...
            int lastConflictPriority = conflictingModModels[
                conflictingModModels.Count - 1
            ].Priority;

            Rectangle imageRect = new(e.Bounds.X + 2 + (16 * iconCount), e.Bounds.Y, 16, 16);
            ...
            {
                throw new Exception(
                    $"ModModel '{Path.GetFileName(modModel.FolderPath)}' is conflicting with '{e.Item.Text}' but unable to determine its conflict status"
                );
            }
        }
    }
    ...
    private void modListView_MouseDoubleClick(object sender, MouseEventArgs e)
    {
        ...
        ModForm modForm = new(_profileModel.GetModModel(listViewItem.Text));
        modForm.FormClosed += (sender, e) => Enabled = true;
        modForm.Show();
    }
    ...
    private void modListViewToolStripMenuItemCreateEmptyMod_Click(object sender, EventArgs e)
    {
        ...
        ListViewItem listViewItem = new(modName);
    ...
    private void modListViewToolStripMenuItemInformation_Click(object sender, EventArgs e)
    {
        ...
        ModForm modForm = new(_profileModel.GetModModel(modListView.SelectedItems[0].Text));
        modForm.FormClosed += (sender, e) => Enabled = true;
        modForm.Show();
    }
