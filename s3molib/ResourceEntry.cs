...
)
{
    ...
    private static readonly List<uint> _textTypes =
    [
    ...
    public Package Package { get; private set; } = package;
    public uint Type { get; private set; } = type;
    public uint Group { get; private set; } = group;
    public uint Id1 { get; private set; } = id1;
    public uint Id2 { get; private set; } = id2;
    public ulong Instance { get; private set; } = (ulong)id1 << 32 | (ulong)id2;
    public uint ChunkOffset { get; private set; } = chunkOffset;
    public uint FileSize { get; private set; } = fileSize & 0x7fffffff;
    public uint MemSize { get; private set; } = memSize;
    public ushort Compressed { get; private set; } = compressed;
    public ushort Unknown2 { get; private set; } = unknown2;

    private byte[] _data;
    public byte[] Data => _data ??= Package.GetUncompressedData(this);
    ...
    public string Decode()
    {
        ...
        else if (Type == 0x0166038C)
        {
            decoded = string.Join(
                Environment.NewLine,
                DecodeNameMap(this)
                    .Select(kvp => Helper.UInt64ToHexString(kvp.Key) + ": " + kvp.Value)
                    .ToArray()
            );
        }
    ...
    public static Dictionary<ulong, string> DecodeNameMap(ResourceEntry resourceEntry)
    {
        ...
        Dictionary<ulong, string> nameMapLookup = [];

        MemoryStream stream = new(resourceEntry.Data);
