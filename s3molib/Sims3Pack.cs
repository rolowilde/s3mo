...
public class Sims3Pack
{
    public static bool Unpack(string path, string targetDirectoryPath)
    {
        ...
        stream.Position = 4;
        string Magic = new(r.ReadChars(7));
        ...
        _ = xmlDocument.SelectSingleNode("/Sims3Package/DisplayName").InnerText;
        List<PackagedFile> packagedFiles = [];

        foreach (XmlNode packaged in xmlDocument.SelectNodes("/Sims3Package/PackagedFile"))
        {
            PackagedFile packagedFile = new();

            foreach (XmlNode content in packaged)
            {
                if (content.Name == "Name")
                {
                    packagedFile.Name = content.InnerText;
                }
                else if (content.Name == "Length")
                {
                    packagedFile.Length = int.Parse(content.InnerText);
                }
                else if (content.Name == "Offset")
                {
                    packagedFile.Offset = long.Parse(content.InnerText);
                }
            }
