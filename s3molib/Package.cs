...
{
    private const uint Magic = 0x46504244; // FPBD
    ...
    public IReadOnlyList<ResourceEntry> ResourceEntries => _resourceEntries;
    ...
    public static Package Load(string path)
    {
        ...
        {
            Logger.LogWarning(
                $"File index version '{indexVersion}' does not match '{IndexVersion}' - {path}"
            );
        }
        ...
        {
            throw new Exception(
                $"File index length '{indexLength}' does not match expected value of '{4 + commonCount * 4 + ((32 - commonCount * 4) * indexCount)}' - {path}"
            );
        }
        ...
        int c = 0;
        Package package = new(path, indexCount, indexLength, indexPosition);
    ...
    )
    {
        ...
        FileStream stream = new(package.FilePath, FileMode.Open, FileAccess.Write, FileShare.Read);
        BinaryWriter w = new(stream);
        ...
        for (int i = source.Count() - 1; i >= 0; i--)
        {
            ...
            {
                if (
                    !resourceKeys.Add(re.ToString())
                    || re.Type == 0x73e93eeb
                    || re.Type == 0x0166038c
                ) // 0x73e93eeb: sims3pack manifest, 0x0166038c: namemap
                {
                    continue;
                }
                ...
                if (stream.Position >= fileSizeLimit)
                {
                    ...
                    package = Package.New(
                        Path.Combine(directoryPath, $"Merge{mergedCount++}.package")
                    );
                ...
                {
                    throw new Exception(
                        $"Unable to obtain data from {Path.GetFileName(importPackage.FilePath)}"
                    );
                }
    ...
    public byte[] GetRawData(ResourceEntry resourceEntry)
    {
        ...
        {
            throw new Exception(
                $"File does not exist anymore since the last time it was opened - {FilePath}"
            );
        }
        ...
        {
            throw new Exception(
                $"File does not contain resource entry the last time it was opened, try call Package.Open() again'{resourceEntry}' - {FilePath}"
            );
        }
    ...
    public byte[] GetUncompressedData(ResourceEntry resourceEntry)
    {
        ...
        if (resourceEntry.FileSize != resourceEntry.MemSize)
        {
            ...
            data = s3pi.Package.Compression.UncompressStream(
                stream,
                (int)resourceEntry.FileSize,
                (int)resourceEntry.MemSize
            );
            stream.Close();
        }
        return data;
    ...
    public bool HasConflictingResourceEntry(
        ResourceEntry resourceEntry,
        out ResourceEntry ownResourceEntry
    )
    {
        ResourceEntry ret = ownResourceEntry = _resourceEntries.Find(r =>
            r.Type.Equals(resourceEntry.Type)
            && r.Group.Equals(resourceEntry.Group)
            && r.Instance.Equals(resourceEntry.Instance)
        );
        return ret != null;
    }
    ...
    public bool AddResourceEntry(ResourceEntry resourceEntry, bool replaceConflicting = true)
    {
        if (HasConflictingResourceEntry(resourceEntry, out ResourceEntry ownResourceEntry))
        {
            ...
            {
                Logger.LogDebug(
                    $"Conflicting resouce key '{resourceEntry.ToString()}', it will be replaced by the latter. '{Path.GetFileName(FilePath)}'    '{Path.GetFileName(resourceEntry.Package.FilePath)}'"
                );
            ...
            {
                Logger.LogDebug(
                    $"Conflicting resouce key '{resourceEntry.ToString()}', the latter will be rejected. '{Path.GetFileName(FilePath)}'    '{Path.GetFileName(resourceEntry.Package.FilePath)}'"
                );
                return false;
            }
        }
    ...
    public bool IsConflictingWith(
        Package package,
        out List<ResourceEntry> conflictedResourceEntries,
        out List<ResourceEntry> conflictedOtherResourceEntries,
        bool fastCheck = false
    )
    {
        conflictedResourceEntries = [];
        conflictedOtherResourceEntries = [];
        foreach (var otherR in package.ResourceEntries)
        {
            if (
                HasConflictingResourceEntry(otherR, out ResourceEntry ownR)
                && ownR.Type != 0x0166038C
                && ownR.Type != 0x73E93EEB
            )
            {
                if (fastCheck)
                {
