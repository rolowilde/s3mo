...
public class ProfileModel
{
    ...
    public List<ModModel> ModModels => new(_modModelLookup.Values);

    public ModModel GetModModel(string modName) => _modModelLookup[modName];

    private readonly Dictionary<string, ModModel> _modModelLookup = [];
    ...
    public void Setup(string profileName)
    {
        ...
        List<string> remainingMods = new(_modModelLookup.Keys);
        int i = 0;
    ...
    {
        List<ModModel> conflictingModModels = new List<ModModel>(_modModelLookup.Values).FindAll(
            m => m.ConflictingModModels.Count > 0
        );
    ...
    public void Save(string profileName)
    {
        ...
        List<ModModel> allModModels = new(_modModelLookup.Values);
    ...
    public void RenameMod(string oldName, string newName)
    {
        ...
        Directory.Move(
            Path.Combine(modsFolderPath, oldName),
            Path.Combine(modsFolderPath, newName)
        );
    ...
    public string CreateEmptyMod()
    {
        ...
        {
            int i = 1;
            while (Directory.Exists(Path.Combine(_exePath, $"Mods\\New Mod ({i})")))
            {
                i++;
            }
